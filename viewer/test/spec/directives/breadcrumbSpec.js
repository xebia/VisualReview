...
describe('Directive: breadcrumb', function () {
  ...
  function getLastSpan(element) {
    return element.find('span:last');
  }
  ...
  it('should render home link when no data is provided', function () {
  ...
  it('should only display a link to the project when project data is given', function () {
    ...
    testHyperLink(element, 0, '#/', '/');
    expect(getLastSpan(element).text()).toBe(scope.myProjectName);
  });
  ...
  it('should display a link to the project and suite when project- and suite data are given', function () {
    ...
    testHyperLink(element, 0, '#/', '/');
    testHyperLink(element, 1, '#/1/', scope.myProjectName);
    expect(getLastSpan(element).text()).toBe(scope.mySuiteName);
  });
  ...
  it('should display a link to the project, suite and run when project-, suite- and run data are given', function () {
    ...
    testHyperLink(element, 0, '#/', '/');
    testHyperLink(element, 1, '#/1/', scope.myProjectName);
    testHyperLink(element, 2, '#/1/2', scope.mySuiteName);
    expect(getLastSpan(element).text()).toBe('3 (Feb 1, 2010 at 12:13PM)');
  });
  ...
  it('should update the link to project when the scope value is updated', function () {
    ...
    testHyperLink(element, 0, '#/', '/');
    testHyperLink(element, 1, '#/11/', scope.myProjectName);
    testHyperLink(element, 2, '#/11/22', scope.mySuiteName);
    expect(getLastSpan(element).text()).toBe('33 (Feb 1, 2011 at 12:13PM)');
  });
});
